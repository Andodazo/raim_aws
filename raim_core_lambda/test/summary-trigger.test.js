...
test('does nothing when summarization is disabled (default)', () => {
  const r = shouldSummarize({ cumulativeInputTokens: 999999, turnCount: 999 }, {});
  assert.equal(r.shouldSummarize, false);
});

test('enabled only when SUMMARIZE_ENABLED=true', () => {
  const r = shouldSummarize({ cumulativeInputTokens: 999999, turnCount: 0 }, ON);
  assert.equal(r.shouldSummarize, true);
});

// ── トークン閾値（主軸）────────────────────

test('triggers when cumulative tokens reach the threshold', () => {
  const env = { ...ON, SUMMARIZE_TOKEN_THRESHOLD: '8000' };
  assert.equal(shouldSummarize({ cumulativeInputTokens: 7999, turnCount: 1 }, env).shouldSummarize, false);

  const hit = shouldSummarize({ cumulativeInputTokens: 8000, turnCount: 1 }, env);
  assert.equal(hit.shouldSummarize, true);
  assert.match(hit.reason, /token_threshold/);
});
...
test('triggers on turn cap even when tokens are low', () => {
  ...
  const hit = shouldSummarize({ cumulativeInputTokens: 500, turnCount: 20 }, env);
  assert.equal(hit.shouldSummarize, true);
  assert.match(hit.reason, /max_turns/);
});

test('token threshold takes priority in the reason', () => {
  const env = { ...ON, SUMMARIZE_TOKEN_THRESHOLD: '8000', SUMMARIZE_MAX_TURNS: '20' };
  const hit = shouldSummarize({ cumulativeInputTokens: 9000, turnCount: 25 }, env);
  assert.match(hit.reason, /token_threshold/);
});

test('does not trigger below both limits', () => {
  const env = { ...ON, SUMMARIZE_TOKEN_THRESHOLD: '8000', SUMMARIZE_MAX_TURNS: '20' };
  assert.equal(shouldSummarize({ cumulativeInputTokens: 1000, turnCount: 3 }, env).shouldSummarize, false);
});
...
test('accumulates across turns until the threshold is crossed', () => {
  ...
  for (let turn = 1; turn <= 6; turn += 1) {
    const thread = { cumulativeInputTokens: 1800 * turn, turnCount: turn };
